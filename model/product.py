...
class Product(BaseModel):
    ...
    def Save(self, typee="other"):
        ...
        if typee == "masive":
            ...
            data = {
                ...
                "sku": self.sku.encode("utf-8"),
                "name": self.name.encode("utf-8"),
                "upc": self.upc.encode("utf-8"),
                "description": self.description.encode("utf-8"),
                "brand": self.brand.encode("utf-8"),
                "manufacturer": self.manufacturer.encode("utf-8"),
                "size": self.size.encode("utf-8"),
    ...
    def get_product_list(self, items=100):
        ...
        url = self.wsurl() + "/product/list?token=" + \
            self.token + "&items={}".format(items)
